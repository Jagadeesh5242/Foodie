...
const fixedScript = `// Fixed JavaScript for search and language functionality
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-image: url('./logodes.jpeg'); background-size: cover; background-position: center; background-attachment: fixed; min-height: 100vh; position: relative; padding-bottom: 80px; }
        ...
        .navbar { background: linear-gradient(90deg, rgba(32,32,46,0.98) 0%, rgba(50,50,80,0.98) 100%); box-shadow: 0 4px 20px rgba(0, 0, 0, 0.3); padding: 0.9rem 0; position: relative; z-index: 100; }
        .navbar-brand { font-size: 1.7rem; font-weight: 900; color: #fff !important; letter-spacing: 1px; transition: color 0.3s ease; }
        .navbar-brand:hover { color: #ffc107 !important; }
        .navbar a { color: #fff !important; margin: 0 0.5rem; transition: all 0.3s ease; font-weight: 600; padding: 8px 16px; border-radius: 20px; }
        .navbar a:hover { color: #ffc107 !important; background: rgba(255, 255, 255, 0.1); }
        .lang-selector { background: rgba(255, 255, 255, 0.15); border: 1px solid rgba(255, 255, 255, 0.3); color: white; padding: 8px 15px; border-radius: 20px; font-weight: 600; cursor: pointer; transition: all 0.3s ease; margin-left: 10px; }
        .lang-selector:hover { background: rgba(255, 255, 255, 0.25); border-color: #ffc107; }
        .lang-selector option { background: #333; color: white; }
        .autocomplete-container { position: relative; display: inline-flex; width: 300px; margin: 0 1rem; }
        #searchInput { padding: 14px 20px; border: none; border-radius: 28px; width: 100%; outline: none; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.2); font-size: 0.95rem; transition: all 0.3s ease; background: white; }
        #searchInput:focus { box-shadow: 0 6px 25px rgba(255, 193, 7, 0.4); transform: scale(1.02); }
        ...
        #suggestionsList li { padding: 14px 20px; cursor: pointer; transition: all 0.2s; border-bottom: 1px solid #f0f0f0; font-weight: 500; }
        #suggestionsList li:hover { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; }
        .recipe-container { max-width: 1000px; margin: 2rem auto; position: relative; z-index: 10; padding: 0 15px; }
        .recipe-header { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 3rem 2rem; border-radius: 20px; text-align: center; margin-bottom: 2rem; box-shadow: 0 8px 30px rgba(0,0,0,0.3); animation: slideInDown 0.6s ease; }
        @keyframes slideInDown { from { opacity: 0; transform: translateY(-30px); } to { opacity: 1; transform: translateY(0); } }
        .recipe-header h1 { font-size: 2.8rem; font-weight: 900; margin-bottom: 0.5rem; text-shadow: 2px 2px 8px rgba(0, 0, 0, 0.3); letter-spacing: 1px; }
        .recipe-header p { font-size: 1.1rem; margin: 0; opacity: 0.95; font-weight: 500; }
        .about-section { background: rgba(255, 255, 255, 0.95); padding: 2rem; border-radius: 15px; margin-bottom: 2rem; box-shadow: 0 4px 15px rgba(0,0,0,0.2); border-left: 5px solid #667eea; }
        .about-section h3 { color: #667eea; font-weight: 700; margin-bottom: 1rem; font-size: 1.3rem; }
        .about-section p { color: #555; line-height: 1.8; font-size: 0.95rem; }
        .ingredients-section { background: rgba(255, 255, 255, 0.95); padding: 2rem; border-radius: 15px; margin-bottom: 2rem; box-shadow: 0 4px 15px rgba(0,0,0,0.2); border-left: 5px solid #ff9800; }
        .ingredients-section h3 { color: #ff9800; font-weight: 700; margin-bottom: 1.5rem; font-size: 1.3rem; display: flex; align-items: center; gap: 10px; }
        .ingredient-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1.5rem; }
        .ingredient-item { display: flex; align-items: center; gap: 12px; padding: 1rem; background: linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%); border-radius: 12px; transition: all 0.3s ease; cursor: default; }
        ...
        .ingredient-icon { font-size: 2rem; flex-shrink: 0; min-width: 50px; text-align: center; }
        .ingredient-text { flex: 1; font-size: 0.95rem; font-weight: 500; line-height: 1.4; }
        .instructions-section { background: rgba(255, 255, 255, 0.95); padding: 2rem; border-radius: 15px; margin-bottom: 2rem; box-shadow: 0 4px 15px rgba(0,0,0,0.2); border-left: 5px solid #4caf50; }
        .instructions-section h3 { color: #4caf50; font-weight: 700; margin-bottom: 1.5rem; font-size: 1.3rem; display: flex; align-items: center; gap: 10px; }
        .instruction-item { display: flex; gap: 15px; margin-bottom: 1.5rem; padding-bottom: 1.5rem; border-bottom: 1px solid #eee; }
        .instruction-item:last-child { border-bottom: none; margin-bottom: 0; padding-bottom: 0; }
        .step-number { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 700; flex-shrink: 0; font-size: 1.1rem; }
        .step-text { flex: 1; color: #444; line-height: 1.8; font-size: 0.95rem; padding-top: 5px; }
        .button-section { display: flex; gap: 1rem; justify-content: center; margin-bottom: 2rem; flex-wrap: wrap; }
        .recipe-btn { padding: 12px 30px; border: none; border-radius: 25px; font-weight: 600; cursor: pointer; transition: all 0.3s ease; font-size: 0.95rem; display: inline-flex; align-items: center; gap: 8px; text-decoration: none; color: white; }
        .btn-home { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); }
        .btn-home:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(102, 126, 234, 0.4); color: white; text-decoration: none; }
        .btn-video { background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); }
        .btn-video:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(245, 87, 108, 0.4); color: white; }
        .footer { position: fixed; bottom: 0; width: 100%; text-align: center; padding: 1.5rem; color: white; background: linear-gradient(90deg, rgba(0,0,0,0.4) 0%, rgba(0,0,0,0.3) 100%); z-index: 10; font-size: 0.95rem; font-weight: 500; letter-spacing: 0.5px; }
        @media (max-width: 768px) {
            .autocomplete-container { width: 100%; max-width: 250px; margin: 0 0.5rem; }
            .recipe-header h1 { font-size: 2rem; }
            .ingredient-grid { grid-template-columns: 1fr; }
            .button-section { flex-direction: column; align-items: stretch; }
        }
    </style>
</head>
<body>
    ...
    <div class="recipe-container">
        ...
        <div class="ingredients-section">
            ...
            <div class="ingredient-grid">
                ...
                <div class="ingredient-item"><div class="ingredient-icon">🧅<\/div><div class="ingredient-text">Aromatics (onion, garlic, ginger)<\/div><\/div>
                <div class="ingredient-item"><div class="ingredient-icon">🫒<\/div><div class="ingredient-text">Oil or Ghee for cooking<\/div><\/div>
                <div class="ingredient-item"><div class="ingredient-icon">🧂<\/div><div class="ingredient-text">Salt to taste<\/div><\/div>
                <div class="ingredient-item"><div class="ingredient-icon">🍅<\/div><div class="ingredient-text">Tomatoes or fresh produce<\/div><\/div>
            <\/div>
        <\/div>
        ...
        <div class="instructions-section">
            ...
            <div class="instruction-item"><div class="step-number">5<\/div><div class="step-text">Cook on medium heat until the dish is fully cooked<\/div><\/div>
...
console.log(`\n==== Fix Complete ====`);
console.log(`Fixed: ${updated} recipe files`);
console.log(`Skipped: ${skipped} files`);
console.log(`\nIssues Fixed:`);
console.log(`✓ Search now clears after navigation`);
console.log(`✓ Can search again immediately`);
console.log(`✓ Language selector improved`);
console.log(`✓ Better Google Translate integration`);
console.log(`✓ Debounced search requests`);
