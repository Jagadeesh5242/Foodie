...
<head>
    <title>RECIPE_NAME - Ruchique 🍽️</title>
    <meta charset="UTF-8">
...
<body>
    ...
    <div class="recipe-container">
        <div class="recipe-header">
            <h1 class="recipe-title">RECIPE_NAME</h1>
        ...
        <div class="action-buttons">
            ...
            <a href="https://www.youtube.com/results?search_query=RECIPE_NAME+recipe" target="_blank" class="btn-action btn-video"><i class="fas fa-play-circle"></i> Watch Recipe Video</a>
        </div>
    </div>
                        ...
                        .then(data => {
                            ...
                            if (data.length > 0) {
                                data.forEach(recipe => {
                                    ...
                                    li.onclick = () => {
                                        const urlFriendly = recipe.toLowerCase().replace(/\\s+/g, '-');
...
function generateIngredientsHtml(ingredients) {
    const emojiMap = {
        'Chicken': '🍗', 'Meat': '🍖', 'Fish': '🐟', 'Shrimp': '🦐',
        'Paneer': '🧀', 'Cheese': '🧀', 'Milk': '🥛', 'Yogurt': '🥛', 'Cream': '🥛',
        'Onion': '🧅', 'Garlic': '🧄', 'Ginger': '🫚', 'Tomato': '🍅', 'Carrot': '🥕',
        'Potato': '🥔', 'Green Chili': '🫑', 'Chili': '🌶️', 'Green Peas': '🫛',
        'Oil': '🫒', 'Ghee': '🫒', 'Butter': '🧈', 'Coconut': '🥥', 'Coconut Milk': '🥥',
        'Rice': '🍚', 'Flour': '🌾', 'Wheat': '🌾', 'Semolina': '🌾',
        'Jaggery': '🟤', 'Sugar': '🟤', 'Honey': '🍯',
        'Almonds': '🌰', 'Cashew': '💰', 'Dry Fruits': '🌰',
        'Spices': '🌶️', 'Salt': '🧂', 'Pepper': '🫑', 'Cumin': '🟤',
    ...
    return ingredients.map(ing => {
        ...
        return `<div class="ingredient-chip">${emoji} ${ing}</div>`;
    }).join('\n                    ');
}
...
function generateHtmlFile(recipe) {
    const ingredientsHtml = generateIngredientsHtml(recipe.ingredients);
    const instructionsHtml = generateInstructionsHtml(recipe.instructions);
    
    let html = baseHtmlTemplate
        .replace(/RECIPE_NAME/g, recipe.name)
        .replace('INGREDIENTS_HTML', ingredientsHtml)
        .replace('INSTRUCTIONS_HTML', instructionsHtml);
...
console.log(`\n📂 Generating ${recipes.length} HTML recipe files...\n`);

recipes.forEach(recipe => {
    try {
        const filePath = path.join(templatesDir, `${recipe.urlName}.html`);
        
        if (fs.existsSync(filePath)) {
            console.log(`⏭️  File already exists: ${recipe.urlName}.html`);
            skipped++;
            return;
        }
        
        const html = generateHtmlFile(recipe);
        fs.writeFileSync(filePath, html, 'utf8');
        console.log(`✅ Created: ${recipe.urlName}.html`);
        created++;
    } catch (error) {
        console.error(`❌ Error creating ${recipe.urlName}.html: ${error.message}`);
        errors++;
    }
});
...
console.log(`\n✨ Generated ${created + skipped} recipe files successfully!`);
